...
Для осуществления мягких проверок следует использовать класс ru.qa.at.assertion.AssertsManager. 
...
В классе ru.qa.at.utils.selenide.extensions.CustomCommands, необходимо релизовать свою логику, например код ниже, будет перед осуществлением клика, прикладывать скриншот элемента, на который будет происходить нажатие.    
```java
...
```
Изменения логики формирования Page объектов, осуществляется в классе ru.qa.at.utils.selenide.extensions.CustomSelenidePageFactory. С помощью этого класса можно расширить/изменить логику инициализации элементов. 
...
```properties
stand= названия стенда для тестирования  в классе (ru.qa.at.utils.Stand) содержатся адреса стендов
